...
public class CapacitorNfcPlugin extends Plugin {
     ...
     * MIFARE Ultralight tags store NDEF in TLV format starting at page 4.
     */
    private NdefMessage readNdefFromMifareUltralight(MifareUltralight mifare) {
        // This code was generated by AI and it's a mess. I don't know how it works. It might be wrong and incorrect.
        // I don't understand any of it and I cannot find its source...
        
        try {
            // Connect immediately - tag can become stale quickly
            mifare.connect();
            
            // Read pages 4-7 in one call (readPages reads 4 pages at a time)
            // This gives us 16 bytes which should cover the TLV header and start of NDEF data
            byte[] pages = mifare.readPages(4);
            ...
            // Get length from TLV header
            int length;
            int dataStartOffset;
            if ((pages[1] & 0xFF) < 0xFF) {
                // Short length format
                length = pages[1] & 0xFF;
                dataStartOffset = 2;
            } else {
                // Extended length format (0xFF followed by 2-byte length)
                if (pages.length < 4) {
                    mifare.close();
                    return null;
                }
                length = ((pages[2] & 0xFF) << 8) | (pages[3] & 0xFF);
                dataStartOffset = 4;
            }
            
            if (length == 0 || length > 512) { // Sanity check
                mifare.close();
                return null;
            }
            
            // Calculate total bytes needed and read all at once if possible
            int totalBytesNeeded = dataStartOffset + length;
            int pagesNeeded = (totalBytesNeeded + 3) / 4; // Round up
            
            byte[] allPages;
            if (pagesNeeded <= 4 && pages.length >= totalBytesNeeded) {
                // We already have all the data from the first read
                allPages = pages;
            } else {
                // Need to read more pages - try to read all at once
                allPages = new byte[Math.min(pagesNeeded * 4, 64)]; // Max 16 pages = 64 bytes
                System.arraycopy(pages, 0, allPages, 0, Math.min(pages.length, allPages.length));
                
                // Read additional pages if needed (pages 8-11)
                if (pagesNeeded > 4) {
                    try {
                        byte[] morePages = mifare.readPages(8);
                        if (morePages != null && morePages.length > 0) {
                            int copyLen = Math.min(morePages.length, allPages.length - pages.length);
                            System.arraycopy(morePages, 0, allPages, pages.length, copyLen);
                        }
                    } catch (Exception e) {
                        // If we can't read more, use what we have
                    }
                }
            }
            
            // Extract NDEF data (skip TLV header)
            if (allPages.length < dataStartOffset + length) {
                mifare.close();
                return null;
            }
            
            byte[] ndefData = new byte[length];
            System.arraycopy(allPages, dataStartOffset, ndefData, 0, length);
            
            mifare.close();
            
            // Parse NDEF message from the raw bytes
            try {
            ...
            // Don't log here to avoid delay - tag is already stale
            try {
                mifare.close();
